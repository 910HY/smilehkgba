...
const PromotionsPage: React.FC = () => {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['/api/promotions', activeTag],
    queryFn: () => activeTag ? getPromotionsByTag(activeTag) : getAllPromotions(),
    retry: 3
  });
  ...
  const handleTagClick = (tag: string) => {
    if (tag === activeTag) {
      ...
      // 否則設置新的篩選標籤
      setActiveTag(tag);
      router.push(`/promotions?tag=${encodeURIComponent(tag)}`, undefined, { shallow: true });
    }
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    if (searchTerm.trim()) {
      router.push(`/promotions?search=${encodeURIComponent(searchTerm.trim())}`, undefined, { shallow: true });
    }
  };
  ...
  const paginate = (pageNumber: number) => {
    ...
    router.push(`/promotions?${queryParams.toString()}`, undefined, { shallow: true });
  };
  ...
  // 獲取當前頁的優惠文章
  const currentPromotions = data ? data.slice(
  ...
  const filteredPromotions = currentPromotions.filter(promotion => {
    ...
    return (
      ...
      promotion.summary.toLowerCase().includes(searchTermLower) ||
      promotion.tags.some(tag => tag.toLowerCase().includes(searchTermLower))
    );
  });
  ...
  if (isLoading) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-12">
        <h1 className="text-3xl font-bold text-white mb-8">牙科優惠資訊</h1>
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 py-12">
        ...
        <div className="bg-red-900 p-6 rounded-lg text-white">
          <h2 className="text-xl font-bold mb-2">載入優惠時發生錯誤</h2>
          <p>抱歉，無法載入優惠內容。請稍後再試。</p>
          <Button 
    ...
    <div className="max-w-7xl mx-auto px-4 py-12">
      ...
      {activeTag && (
        <div className="bg-slate-800 p-4 rounded-lg mb-8 flex justify-between items-center">
          <p className="text-white">
            顯示標籤「<span className="text-orange-400 font-semibold">{activeTag}</span>」的相關優惠
          </p>
          <Button 
            variant="outline" 
      ...
      {filteredPromotions.length > 0 ? (
        ...
        <div className="bg-slate-800 p-6 rounded-lg text-center mb-8">
          <h2 className="text-xl font-semibold text-white mb-2">找不到相關優惠</h2>
          <p className="text-slate-400 mb-4">
            {activeTag 
              ? `沒有與標籤「${activeTag}」相關的優惠。` 
              : router.query.search 
                ? `沒有與「${router.query.search}」相關的搜尋結果。`
                : '目前沒有任何優惠信息。'}
          </p>
          <div className="mt-6 space-y-4">
            <p className="text-white">您可以查看我們的牙科健康文章：</p>
            <Button 
              className="bg-orange-500 hover:bg-orange-600 text-white"
              asChild
            >
              <Link href="/articles">瀏覽牙科健康文章</Link>
            </Button>
          </div>
        </div>
      )}
